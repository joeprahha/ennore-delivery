...
const MyStore = ({ onMenuClick }) => {
  ...
  const fetchOrders = async (selectedStore) => {
    try {
      const response = await api.get(
        `mystore/${selectedStore}/orders?from=${from}&to=${to}`,
        {
          headers: {
  ...
  const playNewOrderSound = () => {
    const phrases = ["Ennore Delivery", "New order", "New order", "New order"];
  ...
  const handleToggle = async () => {
    ...
    try {
      await api.put(`mystore/${updateStore._id}/status?status=${newStatus}`, {
        status: newStatus
      });
  ...
  return (
    ...
    */}
              ...
              {loadingOrders ? (
                ...
                <>
                  ...
                  >
                    ...
                    >
                      {" "}
                      {/* Set font size for all TableCell */}
                      <TableHead>
                        <TableRow>
                          <TableCell
                            sx={{
        ...
        {value === 3 && <Reports selectedStore={selectedStore} />}
        {value === 4 && <Settings selectedStore={selectedStore} />}
      </Box>
    </Box>
  );
};
